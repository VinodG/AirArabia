...
import com.winit.airarabia.adapters.CountryAdapterOfficeNew;
import com.winit.airarabia.objects.OfficeLocationDO;

public class SelectOfficeLocations extends BaseActivity{

	private LinearLayout llSelectCurrency;
	private ListView list;
	private CountryAdapterOfficeNew countryAdapter;
	...
	public void initilize() {
		...
		if (getIntent().hasExtra("vecCountry")) {
			vecCountry = (ArrayList<OfficeLocationDO>) getIntent().getSerializableExtra("vecCountry");
		}
...
//			countryAdapter = new CountryAdapterOfficeNew(SelectOfficeLocations.this, vecCountry,countryNameToCompare);
			countryAdapter = new CountryAdapterOfficeNew(SelectOfficeLocations.this, vecCountry,countryNameToCompare, list,tvNoItem);
	...
	private int getCountryPositionFromString(String countryName)
	{
		...
		if (vecCountry!= null && vecCountry.size()>0) 
		{
			for (int i = 0; i < vecCountry.size(); i++) {
				if (vecCountry.get(i).country.equalsIgnoreCase(countryName)) {
					result = i;
					return result;
				}
			}
